...
import com.kk.android.jetwanandroid.BuildConfig
...
private inline fun kLog(
    ...
    if (Log.isLoggable(tag, level) && isDebug()) {
        if (throwable != null)
            funcThrow(tag, createLog(message?.toString() ?: "<empty info>"), throwable)
        else
            func(tag, createLog(message?.toString() ?: "<empty info>"))
    }
}
